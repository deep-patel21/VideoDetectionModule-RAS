...
def report_eye_aspect_ratio(ax1, df):
    ...
    ear_threshold = df['threshold'].iloc[0]
    ...
    ax1.axhline(y=ear_threshold, color='r', linestyle='--', alpha=0.6, label=f'Eyes Closed Threshold ({ear_threshold:.3f})')
...
def report_observation_status(ax2, df):
    ...
    direction_map = {"DOWN": -2, "LEFT": -1, "FORWARD": 0, "RIGHT": 1, "LOST": 0}
    ...
    ax2.fill_between(df['seconds'], -2.2, 1.2,
                 where=(df['observation_complete'] == 1),
                 color='#27ae60', alpha=0.2, step='post', label='Observation Valid')

    ax2.fill_between(df['seconds'], -2.2, 1.2,
                     where=(df['head_direction'] == "DOWN"),
                     color='red', alpha=0.15, step='post', label='Distracted (Looking Down)')

    ax2.set_yticks([-2, -1, 0, 1])
    ax2.set_yticklabels(['DOWN', 'LEFT', 'FORWARD', 'RIGHT'])
